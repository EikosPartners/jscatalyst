<template>
  <main class="page">
    <section>
      <h1 class="display-1 mt-5 mb-1">Viewport</h1>
      <p class="body-1 mt-3">
    ...
    <section class='viewport-table table-area'>
...
import MockServer from '@/components/Tables/mockServer.js'
...
export default {
  data: function() {
    return {
      ...
      // HTK add back all the editor components and sort/filter parameters once theat's re-implemented
  ...
  computed: {
    ...
    gridOptions: function () {
        ...
        gridOptions.onSortChanged = this.sortChanged;
        gridOptions.onFilterChanged = this.filterChanged;
  ...
  mounted(){
    let localThis = this
    localThis.createViewportDataSource()
  },
  methods: {

    createViewportDataSource: function(item) {
      let localThis = this
      axios.get(this.gridEndpoint).then(response=>{
        ...
        localThis.dataSource = viewportDataSource
        localThis.gridOptions.api.setViewportDatasource(viewportDataSource)
      })
    },
    ...
    sortChanged(event){
      ...
      let connectionID = this.dataSource.connectionId
      let allData = this.dataSource.mockServer.allData
      this.dataSource.mockServer.sortAndFilter(allData, sortModel, filterModel, connectionID)
    },
    filterChanged(event){
      let filterModel = event.api.getFilterModel()
      let sortModel = event.api.getSortModel()
